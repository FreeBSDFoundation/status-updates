...
## **Reporting Period:** 2925-11-26 - 2025-12-02
...
- openjdk25:
  - Submitted fix for incorrect version info in battleblow/jdk25u bsd-port branch:
    - PR: [battleblow/jdk25u PR#6](https://github.com/battleblow/jdk25u/pull/6)
    - Not accepted
  - Updated the jdk25u branch of [freebsd/openjdk](https://github.com/freebsd/openjdk/) with the upstream changes for version 25.0.1, as well as the BSD specific changes, but _not_ with the upstream changes for 25.0.2. This makes the FreeBSD port follow the upstream release tags closer.
  - Tagged [jdk-25.0.1+8-freebsd-1](https://github.com/freebsd/openjdk/releases/tag/jdk-25.0.1%2B8-freebsd-1) relese from the branch above.
  - Submitted updated port for review [D53975](https://reviews.freebsd.org/D53975) - java/openjdk25: Update to version 25.0.1.
- OpenJDK Mainline:
  - Started work on remaining test failures in Servicability Agent
- Misc:
  - Got an emulated Aarch64 install up and running using QEMU, but it's in practice too slow to be very usable. Building OpenJDK takes about 1.5 hours, compared to about 8 min in a similarly speced x86_64 VM. Giving it some more resources will probably help.
- Reviewed [D53944](https://reviews.freebsd.org/D53944) - java/bootstrap-openjdk17: Add missing libX11 dependency.

#### **Completed Tasks**

- Integrated OpenJDK memory alignment fix for non-heap allocations upstream:
  ...
  - The fix is integrated and will be part of the OpenJDK 26 release.
- Published [blog post](https://kodeknekkeriet.net/en/blog/introducing-flavors-in-openjdk-freebsd/) introducing the FLAVORS change to the OpenJDK 25 port, as well as thinking a bit about packaging of OpenJDK on FreeBSD.
  - Got some good responses and pointers [via the fediverse](https://hub.volse.no/channel/harald?mid=a9929344-9bca-4682-91bd-b57fe236612b)
...
- The test failures in the Servicability Agent are relatively difficult to debug, and involve a lot of different layers of code over multiple processes. I have some progressm, but expect this will take most of my focus until I can figure it out.
