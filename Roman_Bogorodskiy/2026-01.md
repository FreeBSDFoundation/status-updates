...
This week I got some reviews on the libvirt arm64 loader handling
patch, and updated the patch to v2. Waiting for reviews on this one and another one
that removes deprecated options.

[The recent changes in -CURRENT](https://cgit.freebsd.org/src/commit/?id=ac5a19ec6989675c8ec6c3ca245dba243d1a6416)
broke libvirt build, so submitted
[a fix](https://lists.libvirt.org/archives/list/devel@lists.libvirt.org/thread/EQQHZLYEK43B4BV7JUINGOEJKDHQ6CXT/)
for that.

Upgraded my CI host from 14.3 to 15.0 and learned that suspend/resume
[does not work well](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=290735#c7)
there.

Started working on CPU pinning configuration support in libvirt for
the bhyve driver.

#### **Completed Tasks**

 * Marked `WITH_BHYVE_SNAPSHOT` as
   [amd64-only](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=292686)
   as a temporary workaround for build failure on arm64.
