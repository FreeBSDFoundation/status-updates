...
Reduced period as was ill towards the end.

### `mac_do(4)`/`mdo(1)`
- Work on the FreeBSD Journal article.
- PR 290352 ("mdo(1) causes kernel panic in jail"): The root cause is I forgot
  to add the code to handle RACCT/RCTL when adding setcred(); the panic does not
  need a jail to be triggered, just having resource accounting on is enough
  (this is not the case on GENERIC, where RACCT code is compiled in but not
  activated by default).  Lead to discovering other bugs and nits.
  - Create D53455 ("racct: Simplify skipping idle process in the throttling daemon")
  - Create D53456 ("kern: Fix credentials leaks on RACCT but no RCTL").
    Discussion with kib@ after its suggestion to avoid relying on
    `proc_set_cred()` and the process lock to keep the passed credentials'
    reference.  Notice that we also rely on that for calls to
    `racct_proc_ucred_changed()`.  Making this change in full however
    complicates the patch, which I intend to MFC to stable/15, so this will be
    applied afterwards (see D53563 below).
  - Create D53457 ("setcred(): Fix RACCT resource accounting on credentials
    change").
  - Commit D53455, D53456 and D53457 to `main`.
  - Create D53563 ("kern: RACCT: Keep process credentials alive via references")
    (see D53456 above for the reason).
