...
## **Reporting Period:** 2025-12-23 - 2025-12-31

Off.


## **Reporting Period:** 2025-12-17 - 2025-12-22
...
- [D49587](https://reviews.freebsd.org/D49587) ("hwpstate: add CPPC support for
  ...
  - Approve newer version.

### MAC framework (mac(4)) / Jails
- Review [D53954](https://reviews.freebsd.org/D53954) ("[RFC] kern: mac: add
  various jail MAC hooks"): Validate.
- Review [D53955](https://reviews.freebsd.org/D53955) ("jaildesc: add an
  accessor for the struct prison in a jaildesc") : Validate.
- Review [D53956](https://reviews.freebsd.org/D53956) ("mac_set_fd(3): add
  support for jail descriptors"): Validate.
- Review [D53958](https://reviews.freebsd.org/D53958) ("kern: add a mac.label
  jail parameter"): A suspect copy-in is in fact required. Suggest moving some
  functions elsewhere. Validate.
- Review [D53959](https://reviews.freebsd.org/D53959) ("libjail: start
  refactoring struct ioctl support") : Glance at it, seems OK and a good idea.
- Review [D53960](https://reviews.freebsd.org/D53960) ("libjail: extend struct
  handlers to included MAC labels") : Spot a small mistake, the rest seems OK.

### `mac_do(4)`/`mdo(1)`
- FreeBSD Journal article: 3+ more passes, finalize.
...
### Misc
- Create [D54271](https://reviews.freebsd.org/D54271) ("jail: Don't define
  malloc type M_PRISON_RACCT on !RACCT") and commit.
- Create [D54284](https://reviews.freebsd.org/D54284) ("kern: racct: Remove code
  unused since %cpu resource computation revamp") and commit.
- Describe the current problems with resource limits that we have when changing
  credentials to `bnovkov@` (resources that should be per-user are in fact
  counted against the current process limit; also, we do not keep track of
  per-process resources assigned to the current user, so are unable to move them
  properly).
