# February 2026 Status Update for Olivier Certner

For the very beginning of February, see previous month's status report.


## **Reporting Period:** 2026-02-18 - 2026-03-03

Was off for a full week, so grouping two periods into one report.

### Power / Hibernate (S4) \[Laptop Project\]
- Preparatory/side changes:
  - Commit
    [d18aaeff8fd314e7](https://cgit.freebsd.org/src/commit/?id=d18aaeff8fd314e7)
    ("vm_page.h: Materialize a spare bit")
  - Commit [D55225](https://reviews.freebsd.org/D55225) ("acpi: Factor out
    message printing on failure of AcpiEnterSleepStatePrep()")
  - Commit [D55226](https://reviews.freebsd.org/D55226) ("acpi: Factor out the
    power off code into acpi_poweroff()")
- Dumping the image
  - Proceed as quickly as possible, at the price of hacks and temporarily
    foregoing refactorings, to reach the milestone of saving an (inconsistent)
    image as an ELF file with specific headers, so that `kib@` can start working
    on the restore part while I'm going off for a week.
  - Dumping works for me under VirtualBox.
  - Send the code through
    <https://github.com/OlCe2/freebsd-src/commits/oc-hibernate/> (head commit:
    [724f1a5](https://github.com/OlCe2/freebsd-src/commit/724f1a5e347b85bfc34793015667ea1d63e53e77)).
  - There is a lot to be resolved before such functionality would become
    commitable (not even talking about image consistency).

### Dynamic P-state support (CPPC) \[Laptop Project\]
- Answer `gallatin@`'s mail about how CPPC works on `freebsd-current@` ("January
  2026 stabilization week" thread).
- Automated benchmarking on `cracauer@`'s machine to test the effect of the
  different CPPC knobs.  Results: Minimum and maximum performance, and EPP
  value, have absolutely no effect.  Only the desired performance has, but with
  high granularity (more than a hundred levels at least, possibly full 256
  ones).
- Commit the following:
  - [D55251](https://reviews.freebsd.org/D55251) ("hwpstate_amd(4): Consistency
    of cached CPPC_REQUEST value")
  - [D55252](https://reviews.freebsd.org/D55252) ("hwpstate_amd(4): CPPC: Allow
    attaching even if CAPABILITY_1 cannot be read")
  - [D55253](https://reviews.freebsd.org/D55253) ("hwpstate_amd(4): CPPC: Switch
    the default to maximum performance")
- On [D55253](https://reviews.freebsd.org/D55253), mention the new
  suspend/resume task (in fact, we have to do something on resume only; prompted
  by the discussion with Rob Augustinus, see previous period).
- Commits:
  - [953b9164c3d9](https://cgit.freebsd.org/src/commit/?id=953b9164c3d9)
    ("cpufreq(4): cpufreq_levels_sysctl(): Remove always false NULL test")
  - [f1b93fc50f01](https://cgit.freebsd.org/src/commit/?id=f1b93fc50f01)
    ("hwpstate_amd(4): Fix punctuation in 'desired_performance' knob's
    description")
- Finally close [PR
  292615](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=292615) ("cpufreq
  changes causes AMD 4750U to run 11x slower").
- Start reviewing:
  - [D55474](https://reviews.freebsd.org/D55474) ("acpi_cppc: Add cppc library
    for parsing ACPI _CPC object.")
  - [D55475](https://reviews.freebsd.org/D55475) ("acpi: Support Functional
    Fixed Hardware")
- Jump to reviewing [D55477](https://reviews.freebsd.org/D55477) ("hwpstate_amd:
  Support writable cpufreq interface in CPPC mode") because this is the actual
  main change and, unfortunately, exactly what I don't want us to do, as this is
  not future-proof (see comment there for reasons and other pointers to earlier
  discussions).
- Review [D55527](https://reviews.freebsd.org/D55527) ("hwpstate_amd: Fix
  autonomous mode"): Reject, as the change reverted here was deliberate (see
  [D55253](https://reviews.freebsd.org/D55253) ("hwpstate_amd(4): CPPC: Switch
  the default to maximum performance")), and while I agree that we should
  probably change the default again, we cannot do it properly without more
  thinking and development since that would basically mean reopening [PR
  292615](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=292615). Hint at
  using the `Preferred_PM_Profile` field of `ACPI`'s `FADT` table to that end.
- Discuss acceptable plans with ShengYi on CPPC but also `powerd(8)` and
  possible scheduler changes.  Likely to organize a mini-hackathon on these
  topics during AsiaBSDCon.

### Laptop Project Admin
- Attend the Power weekly meeting.

### Scheduler
- Review [D55384](https://reviews.freebsd.org/D55384) ("Scheduler and Mutex
  micro optimizations"): Review only the scheduler's part (and one change in
  mutexes); some changes seem interesting (inlining, some data prefetching), but
  more of them look dubious or even useless.

### Review of Others' Work
- [D55341](https://reviews.freebsd.org/D55341) ("procctl(PROC_REAP_KILL): align
  error reporting with pgkill(2)") : Initial change looks wrong; approve after
  correction.
- [D55377](https://cgit.freebsd.org/src/commit/?id=D55377) ("sysctl: Avoid
  calling priv_check() unnecessarily"): Approve.
- [D55378](https://cgit.freebsd.org/src/commit/?id=D55378) ("pipe: Avoid
  unnecessary priv_check() calls in pipespace_new()"): Approve.


## **Reporting Period:** 2026-02-11 - 2026-02-17

Was off for ~1 day.

### Power / Hibernate (S4) \[Laptop Project\]

- Technical debt/preparatory changes/compilation fixes:
  - Commit
    [7e5f4bb3a1c999d1](https://cgit.freebsd.org/src/commit/?id=7e5f4bb3a1c999d1)
    ("kernel dump: dumpsys_gen_pa_next(): Fix "no more chunks" condition
    detection")
  - Create [D55224](https://reviews.freebsd.org/D55224) ("sys: ELF: Rename
    SHN_FBSD_CACHED => SHN_FREEBSD_CACHED").  Commit it.
  - Create [D55225](https://reviews.freebsd.org/D55225) ("acpi: Factor out
    message printing on failure of AcpiEnterSleepStatePrep()")
  - Create [D55226](https://reviews.freebsd.org/D55226) ("acpi: Factor out the
    power off code into acpi_poweroff()")
  - Commit [07c4eb506be4](https://cgit.freebsd.org/src/commit/?id=07c4eb506be4)
    ("sys/compat/freebsd32: Fix i386 compilation"); subsequent discussion with
    `kib@`.
  - Create [D55282](https://reviews.freebsd.org/D55282) ("sys/compat/freebsd32:
    FF clock struct: Don't pack, use 'ffcounter32'").  Commit it.
  - Create [D55283](https://reviews.freebsd.org/D55283) ("sys/abi_types.h:
    time32_t is 64-bit on non-x86 architectures").  Commit it.
- On-going work on dumping the image (see previous period).

### Dynamic P-state support (CPPC) \[Laptop Project\]
- Answer `gallatin@`'s mail about how CPPC works on `freebsd-current@` ("January
  2026 stabilization week" thread).
- More benchmarking on `cracauer@`'s machine to test the effect of the different
  CPPC knobs.
- Related [D55045](https://reviews.freebsd.org/D55045) ("x86: add a safe variant
  of MSR_OP_SCHED* operations") by `kib@`: On-going discussion about allowing
  faults in `*msr_safe()` while interrupts are disabled.
- Commit all revisions between [D55005](https://reviews.freebsd.org/D55005)
  ("hwpstate_amd(4): Register dump: Fine-grained error reporting") and
  [D55010](https://reviews.freebsd.org/D55010) ("hwpstate_amd(4): Add knobs to
  get/set all fields of CPPC_REQUEST") (see the stack).
- Additional commits:
  - [cec0ab05866e](https://cgit.freebsd.org/src/commit/?id=cec0ab05866e)
    ("hwpstate_amd(4): Style: Sort headers")
  - [a78e7c22c5d7](https://cgit.freebsd.org/src/commit/?id=a78e7c22c5d7)
    ("hwpstate_amd(4): Update copyright")
  - [4285340c604a](https://cgit.freebsd.org/src/commit/?id=4285340c604a)
    ("hwpstate_amd(4): Style: Align 'machdep.hwpstate_amd_cppc_enable'")
  - [ebcd80f76ed1](https://cgit.freebsd.org/src/commit/?id=ebcd80f76ed1)
    ("hwpstate_amd(4): Rename PSTATE_CPPC internal flag")
  - [c6a0eb7ada62](https://cgit.freebsd.org/src/commit/?id=c6a0eb7ada62)
    ("hwpstate_amd(4): Rename '*set_autonomous_hwp*()' => 'enable_cppc*()'")
  - [2b728d3bbfbc](https://cgit.freebsd.org/src/commit/?id=2b728d3bbfbc)
    ("hwpstate_amd(4): Move 'struct get_cppc_regs_data' & co. closer to use")
- Create the following revisions:
  - [D55251](https://reviews.freebsd.org/D55251) ("hwpstate_amd(4): Consistency
    of cached CPPC_REQUEST value")
  - [D55252](https://reviews.freebsd.org/D55252) ("hwpstate_amd(4): CPPC: Allow
    attaching even if CAPABILITY_1 cannot be read")
  - [D55253](https://reviews.freebsd.org/D55253) ("hwpstate_amd(4): CPPC: Switch
    the default to maximum performance"); explain the alternatives in an
    additional comment.
- Answer Rob Augustinus' mail on `freebsd-hackers@` about "his" new CPPC driver.
  - Although cleanly written, it has a number of problems (see my answer).
  - We already have this functionality in `hwpstate_amd(4)`, except for
    `powerd(8)` support, which we do not want to do like that.
  - Turns out most of it was probably written by Claude, and we do not want to
    integrate code of dubious/risky provenance.
- Answer Siva Mahadevan's question in [Laptop Project's issue
  108](https://github.com/FreeBSDFoundation/proj-laptop/issues/108).

### Laptop Project Admin
- Attend the Power weekly meeting.
- Open ticket [108](https://github.com/FreeBSDFoundation/proj-laptop/issues/108)
  for the CPPC work.

### Review of Others' Work
- [D55140](https://reviews.freebsd.org/D55140) ("splash: add shutdown splash")


## **Reporting Period:** 2026-02-04 - 2026-02-10

### Dynamic P-state support (CPPC) \[Laptop Project\]
- Factor out `sysctl(9)` knobs and variables common to the Intel and AMD driver
  - Work on a different, simpler approach to ShengYi's
    [D54528](https://reviews.freebsd.org/D54528) ("hwpstate: Use dynamic sysctl
    initialization") where `sysctl(9)` knobs in conflict would be created
    dynamically on attach.
  - In the meantime, Abdelkader Boudih created review
    [D55139](https://reviews.freebsd.org/D55139) ("cpufreq: Fix sysctl name
    conflict between hwpstate_intel and hwpstate_amd") in the meantime, where
    the existing `machdep.hwpstate_pkg_ctrl` is in fact turned into two
    different knobs while giving up on the original name.  State that we do not
    want the old name to disappear for backwards compatibility and that I'm
    already handling the matter.
  - Take over [D54528](https://reviews.freebsd.org/D54528), re-title it to
    "hwpstate{_amd,intel}(4): Move common knobs to a separate file", ask for
    review and finally commit it, along with:
    - Commit
      [1a446f765d9cb34e](https://cgit.freebsd.org/src/commit/?id=1a446f765d9cb34e)
      ("sys/sysctl.h: Make it self-contained when included from the kernel"),
      a prerequisite.
    - Commit
      [bfd50a32ff260675](https://cgit.freebsd.org/src/commit/?id=bfd50a32ff260675)
      ("sys/sysctl.h: Add missing <sys/kassert.h> include") to fix
      `GENERIC-NODEBUG` compilation.
- Commit preliminary revisions:
  - [D54999](https://reviews.freebsd.org/D54999) ("hwpstate_amd(4): Factor out
    printing CPPC registers, fix CPPC_REQUEST's")
  - [D55000](https://reviews.freebsd.org/D55000) ("hwpstate_amd(4): Rename
    register dump sysctl handler")
  - [D55001](https://reviews.freebsd.org/D55001) ("hwpstate_amd(4): Assert
    PSTATE_CPPC is on in corresponding sysctl handlers")
  - [D55002](https://reviews.freebsd.org/D55002) ("hwpstate_amd(4): Rename EPP
    handler, convert to unsigned int")
  - [D55003](https://reviews.freebsd.org/D55003) ("hwpstate_amd(4): attach():
    Fix style")
  - [D55004](https://reviews.freebsd.org/D55004) ("hwpstate_amd(4): Revamp the
    softc")
- Answer `aokblast@`'s comments on reviews
  [D55007](https://reviews.freebsd.org/D55007) ("hwpstate_amd(4): Sane defaults
  for min/max perf on insane capabilities") and
  [D55008](https://reviews.freebsd.org/D55008) ("hwpstate_amd(4): Factor out
  setting the CPPC_REQUEST register"), and make small changes accordingly.
- Note that we are always updating the cached value of the `CPPC_REQUEST`
  register even if writing to it does not succeed.  Start reviewing such kind of
  problems and produce code to fix them.
- [PR 292615](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=292615)
  ("cpufreq changes causes AMD 4750U to run 11x slower"):
  - Following `cracauer@`'s proposal, ask for access to its AMD laptop.
  - Experience a panic caused by some change done in
    [D55005](https://reviews.freebsd.org/D55005) ("hwpstate_amd(4): Register
    dump: Fine-grained error reporting") that we did not see during the review
    so far.
  - Create a change to CPPC initialization to always request maximum
    performance (fixes `cracauer@`'s case).
  - Launch `cracauer@`'s benchmark (SBCL-based) with a variety of CPPC
    parameters (in progress).
- Review related [D55045](https://reviews.freebsd.org/D55045) ("x86: add a safe
  variant of MSR_OP_SCHED* operations") by `kib@`: Doubting this is the proper
  direction, we need more discussion on this.

### Power / Hibernate (S4) \[Laptop Project\]
- Commit
  [53bb02015fce72c7](https://cgit.freebsd.org/src/commit/?id=53bb02015fce72c7)
  ("amd64: parse_memmap(): Move comment about size at proper place").
- Commit revisions:
  - [D54925](https://reviews.freebsd.org/D54925) ("sysctl(9): Booleans: Accept
    integers to ease knob conversion")
  - [D54926](https://reviews.freebsd.org/D54926) ("acpi: Move sysctl tree set up
    at end of initialization")
  - [D54626](https://reviews.freebsd.org/D54626) ("acpi: 'hw.acpi.s4bios'
    sysctl: Change type to bool, expand description")
- Follow-ups to [D54925](https://reviews.freebsd.org/D54925) ("sysctl(9):
  Booleans: Accept integers to ease knob conversion") due to breakage:
  - [PR 292917](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=292917)
    ("machdep.hwpstate_amd_cppc_enable no longer works").
  - Commit [895e1c6567d9](https://cgit.freebsd.org/src/commit/?id=895e1c6567d9)
    ("sysctl(9): Booleans: Fix old value length discovery")
- Commit revision [D54627](https://reviews.freebsd.org/D54627) ("acpi: New R/O
  'hw.acpi.s4bios_supported' sysctl, gate S4BIOS with it"), and small side
  modifications:
  - Commit
    [584c4e0abbfdbac3](https://cgit.freebsd.org/src/commit/?id=584c4e0abbfdbac3)
    ("i386 minidump: Remove unused macros")
  - Commit
    [8bdcfc2a3febcd1a](https://cgit.freebsd.org/src/commit/?id=8bdcfc2a3febcd1a)
    ("endian.h: Visibility comment: Fix grammar, a typo and punctuation")
- Output the image with proper ELF headers
  - Review of dumping code (full dump, "minidump").
  - Review of dump device infrastructure (in particular GEOM code).
  - New `dumpsys_hibernate()` function, a hacked version of `dumpsys_generic()`
    for hibernate (WIP).
  - Discussion with `kib@` on:
    - The format of the segment listing pages to be used by the trampoline.
    - The memory needs (amount, contiguity) to restore the kernel image and jump
      to it.
  - More generally, re-do a pass on the hibernate HackMD document, checking that
    everything makes sense to me (a lot of the second part only vaguely did
    initially).

### Power / Suspend-to-idle (S0ix) \[Laptop Project\]
- Review [D55119](https://reviews.freebsd.org/D55119) ("acpi: Fix trying to
  enter NONE state"): Check for other potential similar mistakes, there are
  none; Approve.

### Laptop Project Admin
- Attend the Power weekly meeting.

### Scheduler
- Respond to some private mail about what "interactivity" means.
- Slight thinking about an alternate but more general design for the unfairness
  problem (problem 2).
- Slight context page-in for Linux's EEVDF.

### `srcmgr@` Lurker
- Review [D55150](https://reviews.freebsd.org/D55150) ("RELNOTES: Add some
  entries for recent commits"): Spot some typo, suggest a change, approve after
  the fixes.

### Review of Others' Work
- [D55082](https://reviews.freebsd.org/D55082) ("riscv: implement kernel ifunc
  resolution"): Answer some `imp@`'s question.
- [D55125](https://reviews.freebsd.org/D55125) ("diff: Report I/O errors in
  Stone algorithm"): Note that some change is gratuitous and makes things less
  readable.
- [D55146](https://reviews.freebsd.org/D55146) ("pwd: Clean up and adopt POSIX
  semantics"): A suspicious test with side-effects is in the end not a bug, but
  is unnecessarily obscure, so suggest changing it and approve the rest.
