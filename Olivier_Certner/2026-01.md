...
- [PR 292615](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=292615)
  ...
  - Send a new patch with a potential fix for `cracauer@`'s case, new knobs to
    control other fields of CPPC (minimum, maximum and desired performance) and
    an updated scale for the `epp` one (0 to 255, the full hardware scale).
  - Request new tests and feedbacks.
  - Answer `aokblast`'s comments:
    - Declaring our frequency to be changeable to the `cpufreq(4)` framework is
      not really possible (relies on frequency <=> CPPC performance value
      mapping, whose existence is dubious, and on setting all minimum, maximum
      and desired value to the same one) and certainly not a good forward path,
      so cannot fix `cy@`'s problem.
    - Limits not being available via ACPI's `_CPC` or even `CPPC_CAPABILITY_1`
      is not really a problem.  On Intel, the SDM says that the values written
      into `CPPC_REQUEST` are always clipped as necessary, and it's likely AMD
      has been doing the same, although the APM is much more vague.  Biggest
      argument is that ACPI says that writing all 0s selects the minimum
      performance, while all 1s selects the maximum one, and AMD processors'
      CPPC support was done to support ACPI's CPPC.
  - `cy@` confirms he can find values for EPP and max performance allowing him
    to recover a behavior similar to what he had with his `powerd(8)`
    configuration (even saying that hints to CPU measurements and controls being
    more sensitive than `powerd(8)`'s).
  - Separate the single big patch in several smaller and more polished commits
    for review, creating revisions [D54996](https://reviews.freebsd.org/D54996)
    ("x86: x86_msr_op(): Add comments, guarantee atomicity") (small spin-off) to
    [D55010](https://reviews.freebsd.org/D55010) ("hwpstate_amd(4): Add knobs to
    get/set all fields of CPPC_REQUEST") (grouped as a stack).
  - Suggest tuning for `cracauer@`.  Seems he can recover full performance by
    setting the desired value to 255 (turning off autonomous mode).
  - Ask `cracauer@` to test a patch corresponding to the new series of
    commits/revisions.  He confirms that patch works (as before).
  - Answer `aokblast@`'s remarks in the created revisions.
  - [D54996](https://reviews.freebsd.org/D54996) ("x86: x86_msr_op(): Add
    comments, guarantee atomicity"): `kib@` seems reluctant to provide the
    atomicity of MSR operation in the `MSR_OP_LOCAL` case, but `markj@` and
    I think that's the safest way for a general interface.
  - Commit [D54996](https://reviews.freebsd.org/D54996) ("x86: x86_msr_op(): Add
    comments, guarantee atomicity") to
    [D54998](https://reviews.freebsd.org/D54998) ("hwpstate_amd(4): Rename CPPC
    register macros").
- Answer `gallatin@`'s email about his fear of our removing of regular P-state
  support (not in my plans).
...
- Create:
  ...
  as new prerequisites for [D54626](https://reviews.freebsd.org/D54626) ("acpi:
  'hw.acpi.s4bios' sysctl: Change type to bool, expand description").
- Update [D54627](https://reviews.freebsd.org/D54627) ("acpi: New R/O
  'hw.acpi.s4bios_supported' sysctl, gate S4BIOS with it") as suggested by
  `obiwac@` to prevent users from setting `hw.acpi.s4bios` to true if
  `hw.acpi.s4bios_supported` is false.
- More work on the code to output the image with proper ELF headers.

### Power / Suspend-to-idle (S0ix) \[Laptop Project\]
- Review [D48735](https://reviews.freebsd.org/D48735) ("acpi: Call SPMC
  suspend/resume hooks"): Suggest renaming the hooks and suppressing tests that
  handler registration worked, as it cannot fail at all.  Hint at updating the
  `EVENTHANDLER(9)` manual page (see next point).
- Review [D55014](https://reviews.freebsd.org/D55014) ("EVENTHANDLER.9:
  EVENTHANDLER_REGISTER never fails"): Approve.

### Laptop Project Admin
- Attend the Power weekly meeting.

### MAC framework (mac(4))
- Review [D54833](https://reviews.freebsd.org/D54833) ("kern: mac: add
  a prison_cleanup entry point"): One suggestion, small discussion with
  `kevans@` about whether we should pass a label as argument of MAC hooks,
  validate.

### Scheduler
- Review [D54951](https://reviews.freebsd.org/D54951) ("tuning.7: document
  schedulers knobs"): Small corrections, approve after modifications.
- Comment on two mails by Steve Kargl:
  - About adding a boot menu option to choose the scheduler: Do not put it in
    the first menu, as we don't have much space there and it should be reserved
    for widely useful tuning.  Warner proposed doing it in the "Advanced Menu",
    which is good.
  - About new tests and numbers: Great that we have them; ULE seems to
    underperform 4BSD in that use case, although not dramatically as was the
    case a long time ago.  This will be a good test case for ULE improvements.
- Review [D55062](https://reviews.freebsd.org/D55062) ("Re-introduce
  kern.sched.topology_spec"): This moves ULE's code which is in fact not
  specific to it to `kern_shim.c`; Approve.
...
- More code analysis and design work
  ...
  - Shutting down the system
    - We absolutely cannot allow the system to write anything to disk after the
      ...
      `kern_reboot()` with `RB_NOSYNC` should be enough.  To check: The shutdown
